        ...
        <div class="mb-6">
            ...
            <form wire:submit.prevent="update">
                <div class="mb-4">
                    ...
                    <input id="name" type="text" wire:model="name" class="form-input mt-1 block w-full" placeholder="Enter permission name">
                ...
                <div class="mb-4">
                    ...
                    @error('feature_id') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                </div>

                <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">
                    Update Permission
                </button>
                <button type="button" wire:click="$set('editMode', false)" class="ml-4 bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">
                    Cancel
                </button>
            </form>
        </div>
        ...
        <tbody>
            @foreach ($permissions as $permission)
                <tr class="border-t hover:bg-gray-50">
                    ...
                    <td class="text-center px-4 py-2 border-b">
                        <button wire:click="edit({{ $permission->id }})" class="bg-yellow-500 text-white px-3 py-1 rounded hover:bg-yellow-600">Edit</button>
                        <button wire:click="delete({{ $permission->id }})" class="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600">Delete</button>
                    </td>
                </tr>
            @endforeach
        </tbody>
